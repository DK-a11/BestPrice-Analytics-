...
export function extractCategoryFromUrl(urlString) {
  try {
    ...
    for (let i = segments.length - 1; i >= 0; i--) {
      ...
      // ⏭️ Пропускаем только технические сегменты
      if (IGNORE_SEGMENTS.includes(segment)) {
        console.log(`  ⏭️ Пропущен: ${segment} (в IGNORE_SEGMENTS)`);
        continue;
      }
      
      // 🔥 НОВАЯ ЛОГИКА: если есть дефис — разбиваем на слова
      if (segment.includes('-') || segment.includes('_')) {
        console.log(`  🔪 Разбиваю "${segment}" по дефису или нижнему подчеркиванию:`);
        const words = segment.split(/[-_]/);
        
        // Проверяем каждое слово из slug
        for (const word of words) {
          console.log(`    - Проверяю "${word}"`);
          
          // Пропускаем короткие и числовые части
        ...
        continue; // Если в slug ничего не нашли, идём дальше
      }
      
      // 🔥 Старая логика для сегментов без дефиса
      if (/\d/.test(segment)) {
        console.log(`  ⏭️ Пропущен: ${segment} (цифры)`);
        continue;
      }
